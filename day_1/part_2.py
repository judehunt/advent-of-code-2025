...
for direction, value in inputs:
    if direction == 'L':
        position, zero_ticks = move_left(position, value)
    elif direction == 'R':
        position, zero_ticks = move_right(position, value)
    log_str = f"The dial is rotated {direction}{value} to point at {position}"
    if zero_ticks > 0:
